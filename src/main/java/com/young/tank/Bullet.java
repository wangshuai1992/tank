 ...
 * @author wangshuai
 * @version V1.0
 * @since 2017-09-29 15:25
 */
public class Bullet extends Material {
    ...
    public Bullet(int img_x, int img_y, int material_x, int material_y, int refurbish, int principal, int direction) {
        ...
        if (principal <= 2){
            new AudioPlay().play("audio\\shoot.wav");//fire
        }
    }
    ...
    public void destroy1(){//子弹出界
        ...
        if (principal <= 2){
            new AudioPlay().play("audio\\hit.wav");
        }
...
//								}
    ...
    public void wounded(Bullet bullet, int principal, int dire, int num) {
        ...
        Data.matarry.remove(this);
        new AudioPlay().play("audio\\hit.wav");//fire
    }
}
